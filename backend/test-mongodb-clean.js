...
async function cleanTestData() {
  try {
    ...
    // Remove existing test user and their dreams
    const testUserId = "test-user-123";

    // Delete dreams first (due to foreign key relationship)
    const dreamsDeleted = await Dream.deleteMany({ userId: testUserId });
    console.log(`🗑️ Deleted ${dreamsDeleted.deletedCount} existing dreams`);

    // Delete test user
    const userDeleted = await User.deleteOne({ firebaseUid: testUserId });
    console.log(`🗑️ Deleted ${userDeleted.deletedCount} existing test user`);
...
async function runTests() {
  console.log("🧪 MongoDB Test Suite for Lucidifier (Clean)\n");
